...
public class MCTS extends Policy {
	...
	public void runSingleMCTS(DecisionNode node) {
		...
		cumulativeReward 					 = simulate(leafState, cumulativeReward, numRollout, numRollout);
		
		// Backpropagation
		backPropagate(leafNode, cumulativeReward);
	}
	 ...
	 * @param cumulativeReward			cumulative reward (initially 0 is given and accumulated as recursion proceeds)
	 ...
	 */
	public double simulate(ArrayList<Boolean> currState, double cumulativeReward, int rolloutHorizons, int remainingRolloutHorizons) {

		// End of simulation, return the cumulative reward
		if (remainingRolloutHorizons == 0) return cumulativeReward;
		...
		cumulativeReward += Math.pow(_discountFactor, rolloutHorizons - remainingRolloutHorizons) * reward;
		
		// Recurse from the next state
		return simulate(nextState, cumulativeReward, rolloutHorizons, remainingRolloutHorizons-1);
	}
	...
	public void backPropagate(TreeNode node, double cumulativeRewardFromLeaf) {
		...
		if (node instanceof StateActionNode) {
			...
			saNode._QVal += cumulativeRewardFromLeaf;
		}
